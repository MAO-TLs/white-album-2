...
test("publishes the inherited clock repairs and the v1.3.1 corrections", async () => {
  ...
  const v131Payloads = await Promise.all(
    ["intro-1009", "intro-1011_030", "closing-2033", "coda-5002", "coda-5102"].map(
      (name) =>
        readFile(new URL(`script-data/${name}.json`, exportRoot), "utf8").then(
          JSON.parse,
        ),
  ...
  const v131Lines = new Map(
    v131Payloads.flatMap((payload) => payload.lines).map((line) => [line.ref, line.english]),
  );
  assert.equal(
    v131Lines.get("wa2:ic:1009:208"),
    "Probably no one at school could have imagined things turning out this way. Two days ago, neither could I.",
  );
  assert.equal(
